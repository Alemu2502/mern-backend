...
export const signin = async (req, res) => {
    ...
    try {
        ...
        // Corrected cookie expiration time handling
        res.cookie('t', token, {
        ...
        res.cookie('refreshToken', refreshToken, {
            expires: new Date(Date.now() + 604800000), // 7 days from now
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
        });

        // Removed localStorage usage from the server-side code
        // localStorage is not available in Node.js
